...
export default function PurchaseInterviews() {
  ...
  const handlePayment = async () => {
    if (!user || !user.email) {
      setError('वापरकर्ता माहिती गहाळ आहे. कृपया पुन्हा लॉगिन करा.');
      return;
    }

    if (interviewCount <= 0) {
      setError('खरेदी करण्यासाठी कृपया किमान १ मुलाखत निवडा.');
      return;
    }
    ...
    try {
      ...
      const options = {
        ...
        name: 'SHAKKTII AI',
        description: `${interviewCount} मुलाखतीची खरेदी`,
        ...
        handler: async function (response) {
          ...
          try {
            ...
            if (!verificationResponse.ok) {
              throw new Error(verificationData.error || 'पेमेंट पडताळणी अयशस्वी झाली');
            }
            ...
            setSuccess(`${interviewCount} मुलाखती यशस्वीरित्या खरेदी केल्या आहेत!`);
            setTimeout(() => {
              router.push('/profile');
            }, 2000);
          } catch (error) {
            console.error('Payment verification error:', error);
            setError(error.message || 'पेमेंट पडताळणी करण्यात अडचण आली');
          } finally {
      ...
      setError(error.message || 'पेमेंट प्रक्रिया सुरू करण्यात अडचण आली');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <>
      <Head>
        <title>मुलाखत खरेदी - SHAKKTII AI</title>
      </Head>
      ...
      <div className="min-h-screen bg-gradient-to-b from-[#1c0032] to-black">
        ...
        <div className="flex justify-between items-center px-6 py-4 bg-[#1a013a] bg-opacity-80 shadow-md">
          ...
          <h1 className="text-xl font-bold text-white">मुलाखत खरेदी करा</h1>
          ...
          <div className="bg-[#29064b] bg-opacity-90 rounded-xl shadow-lg overflow-hidden mb-8 p-6">
            ...
            <div className="mb-6">
              ...
              <p className="text-gray-400 text-xs mt-1">
                एका वेळी जास्तीत जास्त १० मुलाखती घेता येतील
              </p>
            </div>

            <div className="mb-6 p-4 bg-[#1a0035] rounded-lg">
              <div className="flex justify-between mb-2">
                <span className="text-gray-300">प्रत्येक मुलाखतीची किंमत:</span>
                <span className="text-white font-medium">₹१.००</span>
              </div>
              <div className="flex justify-between mb-2">
                <span className="text-gray-300">प्रमाण (नग):</span>
                <span className="text-white font-medium">{interviewCount}</span>
              </div>
              <div className="border-t border-[#3a0a5c] my-2 pt-2"></div>
              <div className="flex justify-between">
                <span className="text-[#e600ff] font-semibold">एकूण:</span>
            ...
            <button
              ...
              className={`w-full py-3 rounded-lg font-bold text-white transition-all ${loading || processingPayment
            ...
            >
              {loading ? (
                ...
                'पेमेंटची पडताळणी होत आहे...'
              ) : (
                `आत्ता ₹${interviewCount.toFixed(2)} भरा`
              )}
            </button>
          </div>

          <div className="bg-[#29064b] bg-opacity-90 rounded-xl p-4 text-gray-300 text-sm">
            <h3 className="text-[#e600ff] font-semibold mb-2">टिप:</h3>
            <ul className="list-disc list-inside space-y-1">
              <li>प्रत्येक मुलाखतीची किंमत ₹१.०० आहे</li>
              <li>मुलाखत क्रेडिट कधीही संपत नाहीत</li>
              <li>पेमेंट Razorpay द्वारे सुरक्षितपणे केले जाते</li>
              <li>चाचणी तपशील (Test credentials): समाप्तीसाठी कोणतीही भविष्यातील तारीख आणि कोणतीही CVV वापरा</li>
            </ul>
          </div>
        </div>
      </div>
