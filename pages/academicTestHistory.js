...
function AcademicTestHistory() {
  ...
  const fetchTestHistory = async (userId) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error || 'चाचणी इतिहास लोड करण्यात अक्षम');
      }
    } catch (error) {
      console.error('Error fetching test history:', error);
      setError('तुमचा चाचणी इतिहास मिळवताना त्रुटी आली. कृपया नंतर पुन्हा प्रयत्न करा.');
    } finally {
      setLoading(false);
    }
  };
    ...
    if (tests.length === 0) {
      return (
        <div className="text-center py-10">
          <div className="text-gray-500 mb-4">तुम्ही अद्याप कोणतीही शैक्षणिक चाचणी घेतलेली नाही.</div>
          <button
            onClick={takeNewTest}
          ...
          <div key={test._id} className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow">
            <div className="flex justify-between items-center">
              ...
              <div className="text-right">
                ...
                <p className="text-xs text-gray-500">वेळ: {formatTime(test.timeSpent)}</p>
              </div>
            </div>
            <div className="mt-3 pt-3 border-t border-gray-100 flex justify-between items-center">
              <span className="text-sm text-gray-500">
                {test.testFormat} • {test.questionCount} {test.questionCount === 1 ? 'प्रश्न' : 'प्रश्न'}
              </span>
              <button
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          >
            ...
            सर्व चाचण्यांच्या यादीकडे परत जा
          </button>
          <div className="flex space-x-2">
            ...
            <span className="px-3 py-1 bg-gray-100 text-gray-800 rounded-full text-sm">
              {selectedTest.questionCount} प्रश्न
            </span>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500">{formatDate(selectedTest.completedAt)} रोजी पूर्ण झाले</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
            <div className="text-sm text-blue-600 font-medium mb-1">स्कोअर</div>
            <div className="text-3xl font-bold text-blue-700">{Math.round(selectedTest.overallScore)}%</div>
          </div>
          <div className="bg-green-50 p-4 rounded-lg border border-green-100">
            <div className="text-sm text-green-600 font-medium mb-1">बरोबर</div>
            <div className="text-3xl font-bold text-green-700">{selectedTest.correctCount || 0}</div>
            <div className="text-xs text-green-500 mt-1">{selectedTest.questionCount} पैकी</div>
          </div>
          <div className="bg-red-50 p-4 rounded-lg border border-red-100">
            <div className="text-sm text-red-600 font-medium mb-1">चुकीचे</div>
            <div className="text-3xl font-bold text-red-700">{selectedTest.incorrectCount || 0}</div>
            <div className="text-xs text-red-500 mt-1">पुनरावलोकन आवश्यक</div>
          </div>
          <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
            <div className="text-sm text-gray-600 font-medium mb-1">घेतलेला वेळ</div>
            <div className="text-3xl font-bold text-gray-700">{formatTime(selectedTest.timeSpent)}</div>
            <div className="text-xs text-gray-500 mt-1">मिनिटे</div>
          </div>
        </div>

        <div className="grid grid-cols-1 gap-6 mb-6">
          <div className="bg-blue-50 p-4 rounded-lg border border-blue-100">
            <h3 className="font-semibold text-blue-800 mb-3">चाचणी सारांश</h3>
            <p className="text-blue-900 mb-4">{selectedTest.feedback}</p>

            {selectedTest.incorrectCount > 0 && (
              <div className="mt-4 pt-4 border-t border-blue-100">
                <h4 className="font-medium text-red-700 mb-2">सुधारणा आवश्यक असलेली क्षेत्रे</h4>
                <div className="space-y-4">
                  {selectedTest.incorrectQuestions.map((q, idx) => (
                    <div key={`incorrect-${idx}`} className="bg-white p-3 rounded-md shadow-sm">
                      <div className="font-medium text-gray-800">प्रश्न: {q.question}</div>
                      <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
                        <div>
                          <span className="text-gray-500">तुमचे उत्तर:</span>
                          <div className="font-medium text-red-600">{q.userAnswer || 'दिलेले नाही'}</div>
                        </div>
                        <div>
                          <span className="text-gray-500">बरोबर उत्तर:</span>
                          <div className="font-medium text-green-600">{q.correctAnswer || 'N/A'}</div>
                        </div>
                      </div>
                      {q.explanation && (
                        <div className="mt-2 text-sm text-gray-700">
                          <span className="font-medium">स्पष्टीकरण:</span> {q.explanation}
                        </div>
                      )}
                      {q.improvementTips && q.improvementTips.length > 0 && (
                        <div className="mt-2">
                          <div className="text-sm font-medium text-gray-700 mb-1">कशी सुधारणा करावी:</div>
          ...
          {selectedTest.subjectMastery && (
            <div className="bg-green-50 p-4 rounded-lg">
              <h3 className="font-semibold text-green-800 mb-2">विषय नैपुण्य (Subject Mastery)</h3>
              {selectedTest.subjectMastery.strengths && selectedTest.subjectMastery.strengths.length > 0 && (
                <div className="mb-3">
                  <h4 className="text-sm font-medium text-green-700 mb-1">कौशल्ये (Strengths)</h4>
              ...
              {selectedTest.subjectMastery.improvements && selectedTest.subjectMastery.improvements.length > 0 && (
                <div className="mb-3">
                  <h4 className="text-sm font-medium text-green-700 mb-1">सुधारणेसाठी क्षेत्रे</h4>
                  <ul className="list-disc list-inside text-green-900 text-sm">
              ...
              {selectedTest.subjectMastery.recommendations && selectedTest.subjectMastery.recommendations.length > 0 && (
                <div>
                  <h4 className="text-sm font-medium text-green-700 mb-1">शिफारसी (Recommendations)</h4>
                  <ul className="list-disc list-inside text-green-900 text-sm">
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>शैक्षणिक चाचणी इतिहास | SHAKKTII AI</title>
        <meta name="description" content="tumcha shaikshanik chachni itihas ani pragati paha" />
      </Head>

      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold text-gray-800">शैक्षणिक चाचणी इतिहास</h1>
          {!showDetail && (
            <button
              onClick={takeNewTest}
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <p className="text-gray-600">तुमचा चाचणी इतिहास लोड होत आहे...</p>
          </div>
        ) : error ? (
