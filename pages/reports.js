...
export default function PremiumReportsPage() {
    ...
    useEffect(() => {
        ...
        const fetchReports = async () => {
            try {
                ...
                setError(err.message || 'रिपोर्ट मिळवण्यात अडचण आली');
            } finally {
                setLoading(false);
            }
        };
    ...
    const getSkillData = (report, skillName) => {
        ...
        return {
            skill: skillName,
            score: scoreValue || 0,
            status: scoreValue >= 8 ? "उत्कृष्ट" : scoreValue >= 5 ? "चांगले" : "दिलेली नाही",
            lastTested: new Date(report.createdAt).toLocaleDateString(),
            progressColor: "#7C3AED",
            summary: `${skillName} साठी कामगिरी विश्लेषण.`,
    ...
    const uniqueRoles = loading ? [] : Array.from(new Set(dateFilteredReports.map(r => (r.role || "अनामित भूमिका").trim().toLowerCase())))
        .map(roleLower => dateFilteredReports.find(r => (r.role || "अनामित भूमिका").trim().toLowerCase() === roleLower).role || "अनामित भूमिका");

    const filteredReports = loading ? [] : dateFilteredReports.filter(report => {
        if (filters.role) {
            return (report.role || "अनामित भूमिका").toLowerCase() === filters.role.toLowerCase();
        }
        return true;
    });

    // Use selectedReport for display, or empty object if none
    const currentViewReport = selectedReport || filteredReports[0] || { role: "उपलब्ध नाही", createdAt: new Date() };
    ...
    const skills = skillsList.map(s => getSkillData(currentViewReport, s) || {
        ...
        score: "-",
        status: "दिलेली नाही",
        lastTested: "उपलब्ध नाही",
        progressColor: "#E2E8F0",
        summary: "माहिती उपलब्ध नाही.",
        detailedFeedback: "तुमचे सविस्तर विश्लेषण पाहण्यासाठी एक मुलाखत द्या.",
    });
    ...
    return (
        <div className="min-h-screen bg-[#F5F5FF]">
            <Head>
                <title>माझे रिपोर्ट | SHAKKTII AI</title>
            </Head>
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                            {loading ? (
                                ...
                                <>
                                    ...
                                    />
                                    <StatCard
                                        label="नोकरीची भूमिका"
                                        value={currentViewReport.role || "उपलब्ध नाही"}
                                        icon={TrendingUp}
                        ...
                        <div className="mb-6">
                            <h2 className="text-xl font-bold text-[#0F172A]">
                                {loading ? (
                                    ...
                                    `कौशल्य विश्लेषण: ${currentViewReport.role || "निवडलेले सत्र"}`
                                )}
                            </h2>
                            <p className="text-sm text-slate-400 mt-1">
                                {loading ? (
                                    ...
                                    `${new Date(currentViewReport.createdAt).toLocaleDateString()} रोजी केलेल्या तुमच्या कामगिरीचे तपशीलवार विश्लेषण`
                                )}
                            </p>
                        </div>
                        ...
                        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6 sticky top-8">
                            ...
                            <div className="space-y-3 max-h-[600px] overflow-y-auto pr-2 custom-scrollbar">
                                {loading ? (
                                    ...
                                    <p className="text-slate-400 text-sm text-center py-8">कोणतेही रिपोर्ट सापडले नाहीत.</p>
            ...
            <DetailedReportModal
                ...
                jobRole={selectedReport?.role || "परस्परसंवादी सत्र"}
