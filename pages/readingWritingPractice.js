...
function ReadingWritingPractice() {
    ...
    try {
      ...
      const processedQuestions = data.questions.map(question => {
        ...
        if (!questionText && question.content) {
          ...
          questionText = `या गोष्टीत ${subject} ने काय केले?`;
        }

        return {
          ...question,
          // Ensure we always have a questionText property that is an actual question
          questionText: questionText || 'या परिच्छेदात काय घडते?',
          // Default timeLimit
          timeLimit: question.timeLimit || 60, // Default to 60 seconds if not specified
          // Ensure we have content
          content: question.content || question.passage || question.text || 'मजकूर काळजीपूर्वक वाचा आणि प्रश्नाचे उत्तर द्या',
          // Default instructions
          instructions: question.instructions || 'परिच्छेद वाचा आणि प्रश्नाचे उत्तर द्या.'
        };
      });
  ...
  return (
    <>
      <Head>
        <title>SHAKKTII AI - {mode === 'reading' ? 'वाचन सराव' : 'लेखन सराव'}</title>
        <meta name="description" content="तुमचे वाचन आणि लेखन कौशल्य सुधारा" />
      </Head>
              ...
              <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                {['बिगिनर', 'मॉडरेट', 'एक्स्पर्ट'].map((level) => (
                  <button
                    ...
                    className={`p-4 rounded-lg font-medium border-2 transition-colors ${difficulty === level
                      ...
                      {levelProgress.map((levelData) => {
                        ...
                        return (
                          <div
                            ...
                            className={`relative p-4 rounded-xl cursor-pointer transition-all transform ${isLocked ? 'bg-gray-200 cursor-not-allowed' :
              ...
              <div className="p-8 text-center">
                ...
                <div className="flex justify-center space-x-4">
                  ...
                  <button
                    ...
                    className={`px-6 py-2 rounded-lg font-medium ${!difficulty || loading
                      ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                      : 'bg-gradient-to-r from-pink-800 to-purple-900 text-white hover:opacity-90'
                      }`}
                  >
                ...
                <div className="flex justify-center space-x-4">
                  ...
                  {selectedLevel < 30 && (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg font-medium ${loading ? 'bg-gray-400 cursor-not-allowed' : 'bg-purple-600 hover:bg-purple-700 text-white'
                        }`}
                      disabled={loading}
                    >
                      {loading ? (
                    ...
                    <div className="space-y-3 mb-4">
                      {questions[currentIndex].options.map((option, index) => (
                        <div
                          ...
                          className={`p-4 rounded-lg cursor-pointer transition-all flex items-center ${selectedOptions.includes(option)
                            ? 'bg-purple-600 text-white border-2 border-purple-800 shadow-md transform scale-[1.02]'
                            : 'bg-gray-100 hover:bg-gray-200 text-gray-800 border border-gray-200'
                            }`}
                        >
                          <div className={`w-6 h-6 flex items-center justify-center rounded-full mr-3 ${selectedOptions.includes(option)
                      ...
                      <div className="ml-3 flex">
                        {[1, 2, 3].map((star) => (
                          <svg
                            key={star}
                            xmlns="http://www.w3.org/2000/svg"
                            className={`h-5 w-5 ${star <= score ? 'text-yellow-500' : 'text-gray-300'
                              }`}
                            viewBox="0 0 20 20"
                            fill="currentColor"
                          >
